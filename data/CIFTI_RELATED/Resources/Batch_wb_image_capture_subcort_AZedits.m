function Batch_wb_image_capture_subcort_AZedits(ciftifile,outname,varargin)
...
%set defaults
image_view = {'Volume_Axial', 'Volume_Coronal', 'Volume_Sagittal_L', 'Volume_Sagittal_R'};
cifti_inds = [];
palette = 'power_surf';
disp_zeros = 'false';
colorscaling = [];
...
for ind = cifti_inds(:)'
    ...
    for n = 1:length(image_view)
        [~,~] = system([workbenchdir 'wb_command -show-scene ' capturefolder 'Hippocampus.scene ' image_view{n} ' ' outname thismapname '_' image_view{n} '.png ' num2str(width) ' ' num2str(height)]);
    end
end
