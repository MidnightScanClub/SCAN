function Batch_wb_image_capture_cort_plussubcort(ciftifile,outname,varargin)
...
% Optional Parameters:
%
%  'image_view' - hemisphere (Left, Right) and number of subcortical slices
%   to be shown in captured images. Omit for default view, or specify one of: 
%   'L' (default), 'R', 'L_Subcort2', 'R_Subcort2', 'L_Subcort4', 'R_Subcort4'
%
...
%parse inputs
attributes = {'image_view','cifti_inds','palette','disp_zeros','colorscaling','colorthresholding','Lsurf','Rsurf','Lsulc','Rsulc'};
...
for ind = cifti_inds(:)'
    ...
    if strcmp(image_view(2:9),'_Subcort')
        height = 990;
        width = 1650;
    else
        height = 700;
    ...
    [~,~] = system(['wb_command -show-scene ' capturefolder '/Capture_cort_plussubcort.scene ' image_view ' ' outname thismapname '_' image_view '.png ' num2str(width) ' ' num2str(height)]);
end
