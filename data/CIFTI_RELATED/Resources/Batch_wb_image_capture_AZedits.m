function Batch_wb_image_capture_AZedits(ciftifile,outname,varargin)
...
workbenchdir = '/data/nil-bluearc/GMT/zhenga/workbench/bin_rh_linux64/';

origcapturefolder = '/data/nil-bluearc/GMT/zhenga/MSC_Subcortical/Scenes/';
...
%set defaults
image_view = 'LR_Cortex_Border';
cifti_inds = [];
palette = 'ROY-BIG';
disp_zeros = 'false';
colorscaling = [];
colorthresholding = [];
Lsurf = '/data/nil-bluearc/GMT/Laumann/MSC/MSC01/master_spec/MSC01.L.inflated.32k_fs_LR.surf.gii';
Rsurf = '/data/nil-bluearc/GMT/Laumann/MSC/MSC01/master_spec/MSC01.R.inflated.32k_fs_LR.surf.gii';
Lsulc = '/data/nil-bluearc/GMT/Laumann/MSC/MSC01/master_spec/MSC01.Lsulc.32k_fs_LR.shape.gii';
Rsulc = '/data/nil-bluearc/GMT/Laumann/MSC/MSC01/master_spec/MSC01.Rsulc.32k_fs_LR.shape.gii';
Lborder = '/data/nil-bluearc/GMT/Scott/MSC_Subcortical/Infomap/MSC01/MSC01_rawassn_minsize400_regularized_filled_recolored_single_cleaned_L.border';
Rborder = '/data/nil-bluearc/GMT/Scott/MSC_Subcortical/Infomap/MSC01/MSC01_rawassn_minsize400_regularized_filled_recolored_single_cleaned_R.border';
volume = '/data/nil-bluearc/GMT/Laumann/MSC/MSC01/T1/MSC01_mpr_debias_avgT_111_t88.nii.gz';

%parse inputs
attributes = {'image_view','cifti_inds','palette','disp_zeros','colorscaling','colorthresholding','Lsurf','Rsurf','Lsulc','Rsulc', 'Lborder', 'Rborder', 'volume'};
...
try copyfile(Lborder,[capturefolder '/L.border']); catch; end
try copyfile(Lborder,[capturefolder '/R.border']); catch; end
...
if strcmp(Rsulc,'none')
    ...
    try copyfile(Lsulc,[capturefolder '/R.sulc.shape.gii']); catch; end
end
...
for ind = cifti_inds(:)'
    ...
    [~,~] = system([workbenchdir 'wb_command -show-scene ' capturefolder 'Hippocampus.scene ' image_view ' ' outname thismapname '_' image_view '.png ' num2str(width) ' ' num2str(height)]);
end

rmdir(capturefolder,'s')
% info = dir([capturefolder 'temp/']);
% info([info.isdir]) = [];
% filenames = fullfile([capturefolder 'temp/'], {info.name});
% delete(filenames{:});
