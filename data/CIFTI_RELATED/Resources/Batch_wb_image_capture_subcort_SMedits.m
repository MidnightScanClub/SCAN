function Batch_wb_image_capture_subcort_SMedits(ciftifile,outname,volume,varargin)
%Batch_wb_image_capture(ciftifile,outname,volume,'param1','value1','param2','value2'...)
%
...
%
%E.Gordon 08/17
% Edited by S Marek (8/17) to loop through all MSC subjects

workbenchdir = '/data/nil-bluearc/GMT/Scott/workbench/bin_rh_linux64/';
...
for ind = cifti_inds(:)'
    ...
    if (exist('palette') && ~isempty(palette)) || (exist('colorscaling') && ~isempty(colorscaling)) || (exist('colorthresholding') && ~isempty(colorthresholding)) || (exist('disp_zero') && ~isempty(disp_zero))
        wbstring = [workbenchdir 'wb_command -cifti-palette ' capturefolder '/cifti.dscalar.nii'];
    ...
    [~,~] = system([workbenchdir 'wb_command -show-scene ' capturefolder '/Capture_subcort.scene ' image_view ' ' outname '_map' num2str(ind) '_' image_view '.png ' num2str(width) ' ' num2str(height)]);
end